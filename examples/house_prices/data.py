...
TARGET_COL = "SalePrice"               # ← change to your target column name
...
def evaluate(pipeline_fn) -> float:
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(cv.split(X, y)):
        ...
        # Apply log1p transformation to training target as requested
        y_tr_log1p = np.log1p(y_tr)
        
        try:
            # y_train passed to pipeline_fn is already log1p transformed
            y_pred = pipeline_fn(X_tr, y_tr_log1p, X_val)
        except Exception as exc:
            sys.exit(f"ERROR in fold {fold_idx}: {exc}")

        y_pred = np.asarray(y_pred, dtype=float)
        # y_val needs to be log1p transformed for the evaluation as well, 
        # since y_pred is in log1p scale (pipeline returns log1p predicted values directly)
        score = -np.sqrt(mean_squared_error(np.log1p(y_val), y_pred))
        scores.append(score)
